...
static double selected_tau_x=3e-8;
static void launch_method(int method,const Param*in,Output*out,size_t n,cudaStream_t stream){
  ...
  else if(method==2)kernel_adaptive<<<blocks,256,0,stream>>>(in,out,n,selected_tau_x,1e-4);
  else kernel_adaptive<<<blocks,256,0,stream>>>(in,out,n,selected_tau_x,1e99);
}
...
int main(int argc,char**argv){
  std::string outdir="results_raw/performance";size_t maxn=2097152;int reps=30,warm=10,heat=5;bool skip_cpu=false;
  for(int i=1;i<argc;i++){if(!strcmp(argv[i],"--out"))outdir=argv[++i];else if(!strcmp(argv[i],"--max-n"))maxn=strtoull(argv[++i],nullptr,10);else if(!strcmp(argv[i],"--repetitions"))reps=atoi(argv[++i]);else if(!strcmp(argv[i],"--warmups"))warm=atoi(argv[++i]);else if(!strcmp(argv[i],"--heat-seconds"))heat=atoi(argv[++i]);else if(!strcmp(argv[i],"--tau-x"))selected_tau_x=strtod(argv[++i],nullptr);else if(!strcmp(argv[i],"--skip-cpu"))skip_cpu=true;}
  ...
  size_t hn=1<<20;std::vector<Param>hp0(hn);for(size_t i=0;i<hn;i++)hp0[i]=make_param(KEPLER,i);Param*hdp;Output*hdo;cudaMalloc(&hdp,hn*sizeof(Param));cudaMalloc(&hdo,hn*sizeof(Output));cudaMemcpy(hdp,hp0.data(),hn*sizeof(Param),cudaMemcpyHostToDevice);double hs=omp_get_wtime();while(omp_get_wtime()-hs<heat)kernel_adaptive<<<int((hn+255)/256),256>>>(hdp,hdo,hn,selected_tau_x,1e-4);cudaDeviceSynchronize();cudaFree(hdp);cudaFree(hdo);
  const char*domains[]={"bem","kepler","pv","cstr","peng_robinson"};const char*methods[]={"fp32","fp64","adaptive_frozen_v3","adaptive_no_gradient_gate_v3"};std::vector<size_t>ns={1,8,32,128,512,2048,8192,32768,131072,524288,2097152,8388608,16777216};
  for(int dom=0;dom<5;dom++)for(size_t n:ns){if(n>maxn)continue;std::vector<Param>p(n);std::vector<Output>ho(n),co(n);for(size_t i=0;i<n;i++)p[i]=make_param(dom,i);cudaHostRegister(p.data(),n*sizeof(Param),cudaHostRegisterDefault);cudaHostRegister(ho.data(),n*sizeof(Output),cudaHostRegisterDefault);Param*dp;Output*doo;cudaMalloc(&dp,n*sizeof(Param));cudaMalloc(&doo,n*sizeof(Output));cudaMemcpy(dp,p.data(),n*sizeof(Param),cudaMemcpyHostToDevice);
    std::vector<double>ct;double cm=NAN;if(!skip_cpu){for(int k=0;k<warm;k++)cpu_run(p,co,128);for(int r=0;r<reps;r++){ct.push_back(cpu_run(p,co,128));raw<<domains[dom]<<','<<n<<",cpu_fp64_omp128,solve,"<<r<<','<<std::setprecision(12)<<ct.back()<<'\n';}cm=median(ct);}
