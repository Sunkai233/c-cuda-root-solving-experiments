#!/usr/bin/env bash
set -euo pipefail
root=/home/abc/supplementary_experiments;cd "$root";inputs="${1:-results_raw/20260824T_bem_condition_scale_inputs_v2}"
out="results_raw/$(date -u +%Y%m%dT%H%M%SZ)_bem_condition_scale_rtx5090";mkdir -p "$out"
printf '%s\n' "$out" > results_raw/LATEST_BEM_CONDITION_SCALE
python3 - "$inputs" <<'PY'
...
for dataset in "$inputs"/baseline_51x*.bin "$inputs"/mixed_51x*.bin "$inputs"/*_524280.bin;do
