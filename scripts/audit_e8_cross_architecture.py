...
def main():
    ap = argparse.ArgumentParser(); ap.add_argument("--root", type=Path, default=Path(".")); ap.add_argument("--run", type=Path, required=True); ap.add_argument("--processed", type=Path, required=True); ap.add_argument("--out", type=Path, required=True); a = ap.parse_args()
    ...
    check("native device identity", summary["device"]["compute_capability"] == "7.0" and summary["device"]["arch"] == "sm_70", summary["device"])
    ...
    check("RTX5090 threshold transfer rejected", not summary["acceptance"]["rtx5090_threshold_transfer_5pct"], summary["routing"]["frozen_max_regret_percent"])
    result = {"scope": "E8 V100 cross-architecture replication", "passed": sum(x["pass"] for x in checks), "total": len(checks), "all_pass": all(x["pass"] for x in checks), "checks": checks}
