...
int main(int argc,char**argv){
  if(argc<2){fprintf(stderr,"usage: %s dataset.bin [stride] [algorithm:0=bisection,1=brent,2=illinois,3=fixed44] [roots.bin]\n",argv[0]);return 2;}
  ...
  for(uint64_t i=0;i<h.n;i+=stride){unsigned node=(unsigned)(i%51)%17;double root;
    int ok=bem_solve_algorithm(vx[i],vy[i],theta[i],hint[i],node,algorithm,&root);if(!ok)fail++;
    double e=fabs(bem_wrap_pi(root-ref[i]));if(e>1e-3)branch++;roots[m]=root;err[m++]=e;
  }
  clock_gettime(CLOCK_MONOTONIC,&t1);double solve_ms=1e3*(t1.tv_sec-t0.tv_sec)+1e-6*(t1.tv_nsec-t0.tv_nsec);
  ...
  printf("{\"algorithm\":%d,\"records_total\":%"PRIu64",\"stride\":%zu,\"evaluated\":%zu,\"solve_ms\":%.6f,\"throughput_roots_s\":%.6f,\"solver_failures\":%zu,\"branch_error_gt_1e-3\":%zu,\"root_abs_rad\":{\"median\":%.17g,\"p95\":%.17g,\"p99\":%.17g,\"max\":%.17g},\"vs_bisection\":{\"gt_1e-8\":%zu,\"gt_1e-6\":%zu,\"p99\":%.17g,\"max\":%.17g}}\n",algorithm,h.n,stride,m,solve_ms,1e3*m/solve_ms,fail,branch,q(err,m,.5),q(err,m,.95),q(err,m,.99),err[m-1],diff1e8,diff1e6,q(delta,m,.99),delta[m-1]);
  free(delta);free(roots);free(err);free(flags);free(buf);return 0;
}
