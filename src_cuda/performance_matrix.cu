...
int main(int argc,char**argv){
  ...
  for(int i=1;i<argc;i++){if(!strcmp(argv[i],"--out"))outdir=argv[++i];else if(!strcmp(argv[i],"--max-n"))maxn=strtoull(argv[++i],nullptr,10);else if(!strcmp(argv[i],"--repetitions"))reps=atoi(argv[++i]);else if(!strcmp(argv[i],"--warmups"))warm=atoi(argv[++i]);else if(!strcmp(argv[i],"--heat-seconds"))heat=atoi(argv[++i]);else if(!strcmp(argv[i],"--tau-x"))selected_tau_x=strtod(argv[++i],nullptr);}
