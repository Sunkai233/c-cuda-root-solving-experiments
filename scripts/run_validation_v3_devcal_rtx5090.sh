#!/usr/bin/env bash
set -euo pipefail
root=/home/abc/supplementary_experiments;cd "$root";tau=3e-8
run_id="$(date -u +%Y%m%dT%H%M%SZ)_validation_v3_devcal_rtx5090";out="results_raw/$run_id";mkdir -p build/cuda_strict "$out"
...
for split in dev cal;do docker run --rm --gpus all --entrypoint bash -v "$root:/work" -w /work -v /usr/lib/x86_64-linux-gnu/libgomp.so.1:/usr/lib/x86_64-linux-gnu/libgomp.so.1:ro nvidia/cuda:12.8.1-base-ubuntu24.04 -lc \
  "build/cuda_strict/validate_raw --references references/ref_v3_20260824 --out '$out' --split '$split' --frozen-only --tau-x '$tau'" | tee "$out/$split.log";done
sha256sum references/ref_v3_20260824/*.csv references/ref_v3_20260824/manifest.json src_cuda/benchmark.cu src_cuda/validate_references.cu >"$out/sha256.txt"
printf '%s\n' "$run_id" > results_raw/LATEST_VALIDATION_V3_DEVCAL
