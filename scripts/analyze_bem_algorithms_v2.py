...
p=argparse.ArgumentParser();p.add_argument('run',type=Path);p.add_argument('--out',type=Path,required=True);a=p.parse_args();names={0:'bisection',1:'brent',2:'illinois',4:'adaptive_compacted'};x={i:json.load(open(a.run/f'performance_alg{i}.json')) for i in names};rows=[]
