...
| C18 | RTX 5090与V100上，冻结正确性和目标预算得到相同通过结果，但混合精度性能与路由阈值不能直接迁移。 | V100完整门0错误接受、自适应0/1000失败、四预算闭合；根+分支证书相对FP64仅`0.403× [0.402,0.403]`，5090冻结路由最坏regret `128.817%`。 | `E8_CROSS_ARCHITECTURE_V100_RTX5090_V1.md` | `results_raw/20260825T224712Z_e8_cross_arch_Tesla_V100-SXM2-32GB_sm_70/`、`results_processed/e8_cross_arch_v1/v100_sm70/` | 当前只有Blackwell消费级与Volta数据中心两类；A100/3090/4090未完成前，不能升级为多代普遍规律。 | 限定后可写 / 性能迁移为负结果 |
...
| C24 | 条件于已知困难比例，层次化路由框架可按硬件校准；RTX 5090冻结阈值不可直接搬到V100。 | RTX 5090自身最大regret 0.128%；搬到V100最大128.817%；V100独立cal选择在九个test点均命中最快模式。 | E12–E16报告、E8阶段报告 | 两卡`routing_summary.csv`与`results_processed/e8_cross_arch_v1/v100_sm70/routing.csv` | 受控基准直接提供`p`，未计在线估计开销；V100本地cal命中不能外推到连续`p`或其他GPU。 | 限定后可写 |
